...
このファイルは`strategy/08_rakuten_api_setup.md`のコピーです。元ファイルは削除・変更していません。
本文中の同フォルダ内参照のみ、移行後のファイル名(`affiliate-link.md`)に更新しています(それ以外の文言は変更していません)。
...
(`affiliate-link.md`の手動リンク変換作業が不要になる)。
...
## 2. アフィリエイトID(affiliateId)を確認する(`affiliate-link.md`の登録が前提)
...
`strategy/08_rakuten_api_setup.md` 全文(同フォルダ内参照`07_affiliate_link_setup.md`→`affiliate-link.md`のみ更新)
